...
from app.models.invoice_model.invoice_model import QuotationReportGenerator,InvoiceReportGenerator
 

def get_quotation_config():
...
def get_invoice_config():
    obj = InvoiceReportGenerator.objects.first()

    if not obj:
        obj = InvoiceReportGenerator.objects.create()
...
class InvoiceReportConfigView(LoginRequiredMixin, View):

    # ✅ GET (fetch config)
    def get(self, request):
        obj = get_invoice_config()
